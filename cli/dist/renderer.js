...
exports.exitAltScreen = exitAltScreen;
exports.render = render;
exports.renderImmediate = renderImmediate;
...
function getWidth() {
    return Math.max(process.stdout.columns || 80, types_js_1.CLI_CONFIG.MIN_WIDTH);
}
...
function truncateFilePath(filePath, maxLen) {
    if (filePath.length <= maxLen)
        return filePath;
    const parts = filePath.split('/');
    if (parts.length <= 2)
        return truncate(filePath, maxLen);
    // Keep filename and abbreviate directories
    const filename = parts[parts.length - 1];
    if (filename.length >= maxLen - 4)
        return truncate(filename, maxLen);
    let result = '…/' + filename;
...
// Tier 1: Org Overview
// ============================================
function renderTier1(state, width) {
    const lines = [];
    const { tier1 } = state;
...
function renderMember(member, width, isNested) {
...
function renderTier2(state, width) {
    const lines = [];
    const { tier2 } = state;
    lines.push('');
...
// ============================================
function renderTier3(state, width) {
    const lines = [];
    const { tier3 } = state;
    lines.push('');
    ...
    const displayEvents = tier3.events.slice(-types_js_1.CLI_CONFIG.ACTIVITY_DISPLAY_MAX);
...
function render(state) {
    ...
    renderTimeout = setTimeout(() => {
        renderTimeout = null;
        renderFrame(state);
    }, types_js_1.CLI_CONFIG.RENDER_DEBOUNCE);
}
/**
