...
class DecorationManager {
    ...
    updatePresence(states, currentUserId) {
        ...
        states.forEach((state) => {
            if (state.userId === currentUserId)
                return; // Skip self
            if (state.status === 'offline' || state.status === 'draft')
                return;
    ...
    getTeammatesInFile(relativePath) {
